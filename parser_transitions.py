...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        """Performs a single parse step by applying the given transition
         to this partial parse
        ...
        if transition == "S":
            ...
            print("transition is not a legal transition")
...
if __name__ == '__main__':
    args = sys.argv
    # model = DummyModel()
    # sentences = [["right", "arcs", "only"], ["right", "arcs", "only", "again"],
    #              ["left", "arcs", "only"],
    #              ["left", "arcs", "only", "again"]]
    # parses = [PartialParse(sentence) for sentence in sentences]
    # while len(parses) > 0:
    #     pred_tran = model.predict(parses)
    #     # print(pred_tran)
    #     copy = [parse for parse in parses]
    #     for index, single_parse in enumerate(copy):
    #         # print("index = ", index,
    #         #       "pred_tran[index] = ", pred_tran[index])
    #         single_parse.parse([pred_tran[index]])
    #         # print("buffer", single_parse.buffer,
    #         #       "stack", single_parse.stack, len(single_parse.stack))
    #         if len(single_parse.buffer) == 0 and \
    #                 len(single_parse.stack) == 1:
    #             # print("to be removed")
    #             parses.remove(single_parse)
    if len(args) != 2:
        raise Exception(
